...
if(Modernizr.webgl) {
	...
	function ready (error, data, config, geog){
		...
		areas.features.map(function(d,i) {//if(d.properties.AREACD!="E09000001"){
		  ...
		  d.properties.fill2 = color(dataById[d.properties.AREACD]["yr2"])//colours for second map
		});
...
d3.select("#mapRight").style("top","130px")
		...
		function setAxisVal(code) {
			...
			d3.select("#currVal")
				.text(function(){if(!isNaN(rateById[code]))  {return "2016: "+displayformat(rateById[code])} else {return "Data unavailable"}})
				.style("opacity",1)
				.transition()
				.duration(400)
			...
			d3.select("#currLineRight")
				.style("opacity", function(){if(!isNaN(dataById[code]["yr2"])) {return 1} else{return 0}})
				.transition()
				.duration(400)
				.attr("x1", function(){if(!isNaN(dataById[code]["yr2"])) {return x(dataById[code]["yr2"])} else{return x(midpoint)}})
				.attr("x2", function(){if(!isNaN(dataById[code]["yr2"])) {return x(dataById[code]["yr2"])} else{return x(midpoint)}});


			d3.select("#currValRight")
				.text(function(){if(!isNaN(rateById[code]))  {return "2026: "+displayformat(dataById[code]["yr2"])} else {return "Data unavailable"}})
				.style("opacity",1)
				.transition()
				.duration(400)
				.attr("x", function(){if(!isNaN(dataById[code]["yr2"])) {return x(dataById[code]["yr2"])} else{return x(midpoint)}});
		...
		function createKey(config){
			...
			x = d3.scaleLinear()
				...
				.range([0,keywidth-65]); /*range for pixels*/
